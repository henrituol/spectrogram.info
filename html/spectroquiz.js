...
.then(utilizeFetchedData)           // Take the data to good use. See function below.
.catch(console.error);
...
function randomizeSpectrograms () {
    ...
    //  -- Or not! Seems like option C is preferred at the beginning.
...
function createQuizPage(audioFile, image1, image2, image3, whichOneIsRight) {
    ...
    // Select song from the local folder
    ...
    function selectionResult(checkboxId, whichOneIsRight) {
        if (checkboxId == whichOneIsRight) {
            ...
            let answer = "You chose " + checkboxId + ". " + "The correct answer is " + whichOneIsRight + "."; 
            document.getElementById("result").innerHTML=answer;
        }
    }

    // Check, if there is already a selection symbol in another changebox.
